...
public class IntakeCommand extends SequentialCommandGroup {
    ...
    public IntakeCommand() {
        Pose2d nearestNote = Swerve.get().getEstimatedPose().nearest(Arrays.asList(new Pose2d())); // TODO: Insert note pose getter here

        if (Math.abs(Swerve.get().getEstimatedPose().getX() - nearestNote.getX()) < DriveConstants.ALLOWED_DISTANCE_FROM_NOTE.getX() && Math.abs(Swerve.get().getEstimatedPose().getY() - nearestNote.getY()) < DriveConstants.ALLOWED_DISTANCE_FROM_NOTE.getY()) {
            addCommands(
