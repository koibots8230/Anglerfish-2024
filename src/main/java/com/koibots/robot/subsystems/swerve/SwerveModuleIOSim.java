...
public class SwerveModuleIOSim implements SwerveModuleIO {
    ...
    private final DCMotorSim driveSim = new DCMotorSim(DCMotor.getNEO(1), DriveConstants.DRIVE_GEAR_RATIO, 0.025);
    private final DCMotorSim turnSim = new DCMotorSim(DCMotor.getNEO(1), DriveConstants.TURN_GEAR_RATIO, 0.004);
    ...
    public void updateInputs(SwerveModuleInputs inputs) {
        ...
        inputs.driveVelocity = DriveConstants.WHEEL_RADIUS.times(driveSim.getAngularVelocityRadPerSec()).per(Second);
        inputs.driveAppliedVoltage = driveAppliedVolts;
        inputs.driveCurrent = Amps.of(driveSim.getCurrentDrawAmps());

        inputs.turnPosition = new Rotation2d(turnSim.getAngularPositionRad() % (2 * Math.PI)).minus(Rotation2d.fromRadians(Math.PI));
